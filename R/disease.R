...
#'   compressed.
#' @param platform Optional platform index, GPL accession, or annotation name
#'   when the Series Matrix contains more than one ExpressionSet.
#' @return A list containing an expression matrix, phenotype table, platform
#'   annotation name and source checksum.
#' @export
read_geo_series <- function(series_matrix, platform = NULL) {
  ...
  if (methods::is(obj, "ExpressionSet")) {
    ...
    if (length(obj) > 1L && is.null(platform)) {
      labels <- vapply(obj, Biobase::annotation, character(1L))
      stop("The Series Matrix contains multiple platforms: ",
           paste(labels, collapse = ", "),
           ". Specify platform explicitly.", call. = FALSE)
    }
    if (is.null(platform)) {
      eset <- obj[[1L]]
    } else if (is.numeric(platform)) {
      eset <- obj[[as.integer(platform[[1L]])]]
    } else {
      labels <- vapply(obj, Biobase::annotation, character(1L))
      hit <- which(tolower(labels) == tolower(as.character(platform[[1L]])))
      if (length(hit) != 1L) {
        stop("Requested platform was not uniquely identified. Available: ",
...
normalize_expression <- function(expr, method, force_log2 = NA) {
  ...
  if (method == "none") {
    attr(expr, "proret_normalization") <- list(
      method = method, log2_applied = FALSE, force_log2 = force_log2
    )
    return(expr)
  }
  ...
  attr(expr, "proret_normalization") <- list(
    method = method, log2_applied = do_log, force_log2 = force_log2,
    input_quantiles = stats::setNames(as.numeric(qx), names(qx))
  )
  expr
}
...
fit_disease_signature <- function(
  ...
  normalization_audit <- attr(expr, "proret_normalization")
...
#' @param force_log2 Override automatic log2 detection.
#' @param platform Optional platform selector for multi-platform input.
...
prepare_disease_from_series <- function(
    ...
    covariates = character(),
    normalization = "microarray_quantile",
    force_log2 = NA, platform = NULL,
    output_dir = NULL) {
  geo <- read_geo_series(series_matrix, platform = platform)
  pd <- geo$phenotype
