...
project_disease_signature <- function(disease, basis, core_genes,
  ...
  out <- list(
    ...
    projection_checksum = basis_projection_checksum(basis, core),
...
score_reversal <- function(
  ...
  if (!identical(disease_projection$basis_checksum,
    ...
    if (is.null(disease_projection$projection_checksum) ||
        is.null(drug_programs$projection_checksum)) {
      stop("Disease and drug projections use different basis checksums.",
           call. = FALSE)
    }
  }
  if (is.null(drug_programs$projection_checksum) ||
      !identical(disease_projection$projection_checksum,
                 drug_programs$projection_checksum)) {
    stop("Disease and drug projections use different core-basis checksums.",
         call. = FALSE)
  }
  observed_payload <- drug_payload_checksum(
    drug_programs$signatures, drug_programs$core_genes,
    drug_programs$program_names, drug_programs$projection_checksum
  )
  if (is.null(drug_programs$payload_checksum) ||
      !identical(observed_payload, drug_programs$payload_checksum)) {
    stop("Drug-program reference failed its payload integrity check.",
         call. = FALSE)
  }
...
#' @param cell_panel_policy Rule used to define the disease-independent cell
#'   panel: coverage-derived, all observed, or explicitly configured.
#' @param min_cell_panel_coverage Minimum drug coverage used to retain a cell
#'   in the coverage-derived panel.
#' @param min_drug_panel_fraction Minimum fraction of the selected panel
#'   required for a drug.
...
#' @param cell_panel_policy,min_cell_panel_coverage,min_drug_panel_fraction
#'   Disease-independent cell-panel settings.
